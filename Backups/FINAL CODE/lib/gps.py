...
from machine import reset, UART, Pin  # Importerer nødvendige klasser til UART-kommunikation og reset-funktion
from gps_simple import GPS_SIMPLE  # Importerer GPS_SIMPLE-biblioteket til at arbejde med GPS-data

# Konfiguration af GPS
gps_port = 2  # Angiver ESP32 UART-porten (Educaboard ESP32 bruger typisk port 2)
gps_speed = 9600  # Angiver UART-baudrate (standard for u-blox GPS-enheder)

# Initialisering af UART og GPS
uart = UART(gps_port, gps_speed)  # Initialiserer UART med den angivne port og hastighed
gps = GPS_SIMPLE(uart)  # Opretter GPS-objekt ved hjælp af GPS_SIMPLE-biblioteket
