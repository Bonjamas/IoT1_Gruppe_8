from machine import reset, Pin, I2C, SoftI2C
...
imu = MPU6050(i2c)
Softi2c = SoftI2C(scl=Pin(18), sda=Pin(19), freq=400000)
ina = INA219(Softi2c)
...
try:
    while True:
        ...
        if alarm_enabled or solenoid_enabled:
            ...
            if imu_data.get("acceleration x", 0) < -5000:  # Handle missing data safely
                alarm()
            else:
                np_clear()
        else:
        ...
        try:
            ...
            remaining_time = (battery_percent * 1800) / current if current > 0 else float("inf")
        except Exception as e:
            print("INA219 error")
            current, voltage, battery_percent, remaining_time = 0, 0, 0, 0
