...
while True:
    try:
        client.check_msg()
        if imu_data.get("acceleration y") < 100 and imu_data.get("acceleration y") > -100 and check_movement:
            timer = ticks_ms()
            check_movement = False
        if imu_data.get("acceleration y") > 100 and imu_data.get("acceleration y") < -100 and not check_movement:
            check_movement = True
            send_data = True
        if not check_movement and ticks_ms() - timer > 5000:
            send_data = False
