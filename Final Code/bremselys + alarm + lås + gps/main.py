...
import gc
import secrets
from functions import initialize_imu, set_brake_light, check_brake, np_clear, alarm, control_solenoid, get_gps_location
...
try:
    while True:
        ...
        # Check if the bike has been still for more than 3 minutes
        if imu.get_values()["acceleration x"] == 0:
            bike_still_time += 1
        else:
            bike_still_time = 0

        if bike_still_time > 18:  # 18 * 10ms = 180s (3 minutes)
            latitude, longitude = get_gps_location()
            client.send_attributes({'location': {'latitude': latitude, 'longitude': longitude}})
